...
function tipoMultimedia(item) {
  ...
  if (f.includes("twitter") || f.includes("x.com") || f.includes(" x ")) return "twitter";
...
function itemsFiltrados() {
  ...
  return items.filter(i => {
    ...
    const ts = i.ts;
    const okSemana = !ts || itemEnSemana(new Date(ts).getTime(), store.get("semanaNoticias"));
    const okCanal = store.get("canalNoticias") === "all" || limpiarFuente(i.fuente) === store.get("canalNoticias");
...
function actualizarCategorias() {
  ...
  const cats = Object.entries(
    items.reduce((acc, i) => { const c = i.categoria; if (c) acc[c] = (acc[c] || 0) + 1; return acc; }, {})
  ).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([cat]) => cat);
  const active = store.get("catNoticias");
  if (active !== "all" && !cats.includes(active)) store.set("catNoticias", "all");
  container.innerHTML = "";
  const opts = [{ id: "all", label: "Todas" }, ...cats.map(c => ({ id: c, label: c }))];
  opts.forEach(opt => {
    const chip = document.createElement("div");
    chip.className = "chip" + (store.get("catNoticias") === opt.id ? " active" : "");
...
function renderNoticias(items) {
  ...
  ul.innerHTML = items
    ...
    .map(i => {
      ...
      return `<li class="news-item" data-ts="${ts}" data-fuente="${fuente}" data-categoria="${cat}" data-origen="${i.origen || "web"}">
...
function renderYouTube(items, container) {
  ...
  container.innerHTML = videos.map(i => {
    ...
    const clase = esLive ? "tipo-live" : tipo === "shorts" ? "tipo-shorts" : `tab-${tab}`;
...
function renderTrends(items) {
  ...
  container.innerHTML = trends.map(i => {
    const ts = i.ts || "";
...
function registrarEventos() {
  ...
  store.on("semanaNoticias", () => { aplicarFiltrosNoticias(); actualizarCategorias(); });
  store.on("canalNoticias", () => { aplicarFiltrosNoticias(); actualizarCategorias(); });
  store.on("catNoticias", () => { aplicarFiltrosNoticias(); actualizarCategorias(); });
